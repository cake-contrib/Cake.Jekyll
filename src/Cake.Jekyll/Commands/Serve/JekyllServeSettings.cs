...
/// <summary>
/// Contains settings used by <see cref="JekyllServeCommand"/>.
/// </summary>
public class JekyllServeSettings : JekyllSettings
{
    /// <summary>
    /// Initializes a new instance of the <see cref="JekyllServeSettings"/> class.
    /// </summary>
    ...
    /// <summary>
    /// X.509 (SSL) Public Certificate, stored or symlinked in the site source.
    /// --ssl-cert [CERT]
    /// </summary>
    public FilePath SslCertificate { get; set; }

    /// <summary>
    /// X.509 (SSL) Private Key, stored or symlinked in the site source.
    /// --ssl-key [KEY]
    /// </summary>
    public FilePath SslKey { get; set; }
    ...
    /// <summary>
    /// Open the site's URL in the browser (defaults to <see langword="false"/>).
    /// -o, --open-url
    /// </summary>
    public bool? OpenUrl { get; set; }

    /// <summary>
    /// Detach the server from the terminal (defaults to <see langword="false"/>).
    /// -B, --detach
    /// </summary>
    public bool? Detach { get; set; }

    /// <summary>
    /// Show a directory listing instead of loading your index file (defaults to <see langword="false"/>).
    /// --show-dir-listing
    /// </summary>
    public bool? ShowDirListing { get; set; }

    /// <summary>
    /// Skips the initial site build which occurs before the server is started (defaults to <see langword="false"/>).
    /// --skip-initial-build
    /// </summary>
    public bool? SkipInitialBuild { get; set; }

    /// <summary>
    /// Reload a page automatically on the browser when its content is edited (defaults to <see langword="false"/>).
    /// -l, --livereload
    /// </summary>
    ...
    /// </summary>
    public OneOrMoreStrings LiveReloadIgnore { get; set; }

    /// <summary>
    /// Minimum delay before automatically reloading page.
    /// --livereload-min-delay SECONDS
    /// </summary>
    public TimeSpan? LiveReloadMinDelay { get; set; }

    /// <summary>
    /// Maximum delay before automatically reloading page.
    /// --livereload-max-delay SECONDS
    /// </summary>
    public TimeSpan? LiveReloadMaxDelay { get; set; }
    ...
    protected override void EvaluateCore(ProcessArgumentBuilder args)
    {
        ...
        ApplyOption(args, "--ssl-cert", SslCertificate?.FullPath);
        ApplyOption(args, "--ssl-key", SslKey?.FullPath);
        ApplyOption(args, "--host", Hostname);
        ApplyOption(args, "--port", Port);
        ApplyOption(args, "--open-url", OpenUrl);
        ApplyOption(args, "--detach", Detach);
        ApplyOption(args, "--show-dir-listing", ShowDirListing);
        ApplyOption(args, "--skip-initial-build", SkipInitialBuild);
        ApplyOption(args, "--livereload", LiveReload);
        ApplyOption(args, "--livereload-ignore", LiveReloadIgnore);
        ApplyOption(args, "--livereload-min-delay", LiveReloadMinDelay);
        ApplyOption(args, "--livereload-max-delay", LiveReloadMaxDelay);
        ApplyOption(args, "--livereload-port", LiveReloadPort);
